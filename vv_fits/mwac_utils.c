			...
			for (pol1 = 0; pol1 < npol; pol1++) {
				for (pol2 = 0; pol2 < npol; pol2++) {
					...
					/*
					   ...
					   pfb_output_to_input[out1],
					   pfb_output_to_input[out2]);
				    */
					corr_mapping[pfb_output_to_input[out1]][pfb_output_to_input[out2]].stn1 =
						stn1; // this should give us the pfb input
					corr_mapping[pfb_output_to_input[out1]][pfb_output_to_input[out2]].stn2 =
						stn2;
					corr_mapping[pfb_output_to_input[out1]][pfb_output_to_input[out2]].pol1 =
						pol1;
					corr_mapping[pfb_output_to_input[out1]][pfb_output_to_input[out2]].pol2 =
						pol2;
			...
			for (j = 0; j <= i; j++) {
				for (pol1 = 0; pol1 < npol; pol1++) {
					for (pol2 = 0; pol2 < npol; pol2++) {
						...
						out_index = f*(nsta*nsta*npol*npol) + 
                                    i*(nsta*npol*npol) + 
                                    j*(npol*npol) + pol1*(npol) + pol2;

						out_index_conj = f*(nsta*nsta*npol*npol) + 
                                           j*(nsta*npol*npol) + 
