 ...
 *
 * Build executable:
 * $ gcc -g  -lcfitsio -lm mwac_utils.c build_lfiles.c antenna_mapping.c -o build_lfiles
 * 
 * Example:
 * $ ./build_lfiles -d -m 1 -v 1130642936_20151104032841_gpubox11_00.fits -o 1130642936_20151104032841_gpubox11_00
 * 
 *
...
void usage() {
    ...
    fprintf(stdout,"create lfiles (autos and X) from a correlator dump (mode 0) " \
            "or an NGAS FITS file (mode 1)\n");
    ...
    fprintf(stdout,"\t-v\t <visibility file name> -- this argument can appear multiple times. " \
            "Each file is concatenated in frequency <NOT TIME>\n");
    fprintf(stdout,"\t-f\t <nfreq> [default: determine from input filess]: " \
            "Number of channels <must equal nfiles*nchan_per_file>\n");
    fprintf(stdout,"\t-i\t <number of inputs> [default: determine from input files] : " \
            "number of input that have been correlated <nsta*npol>\n");
    fprintf(stdout,"\t-o\t <output filename root> [default == \"last_dump\"] " \
            "root filename of the output\n");
    ...
    fprintf(stdout,"\t-F\t <factor> [default == 1] -- by what factor to average in frequency " \
            "(must be power of 2)\n");
    fprintf(stdout,"\t-P\t <type>   Output product type. A: autos, " \
            "C: cross, B: both. Default: both\n");
...
int getHDUsInfo(int nfiles, fitsfile *fptr[MAX_FILES], int num_hdus_in_file[MAX_FILES], 
                long dims[2]) {
    ...
    for (i=1; i< nfiles; i++) {
        if ( fabs((num_hdus_in_file[i]-num_hdus_in_file[i-1])/(num_hdus_in_file[i]+1.0)) > 0.1) {
            fprintf(stderr,"ERROR: serious mismatch between number of " \
                    "HDUs in files %s (%d) vs %s (%d). "                \
                    "Exiting.\n", input_file[i],num_hdus_in_file[i], 
                    input_file[i-1], num_hdus_in_file[i-1]);
            return EXIT_FAILURE;
        }
    }
...
void allocate_arrays(size_t matLength, size_t lccspcLength, size_t lacspcLength, size_t fullLength,
                     ...
                     float complex **lccspc_h, float **lacspc_h, float complex **lcc_accum, 
                     float **lac_accum){
...
//    extern char *optarg;  // This is in getopt.h
    ...
    while ((arg = getopt(argc, argv, "m:i:f:F:s:v:n:o:apT:P:d")) != -1) {
        ...
        switch (arg) {
            ...
            case 'F':
                ...
                break;
            case 's':
                start_sec = atoi(optarg);     /* In fact, start_sec means starting_number_of_hdu ???????????????? */
                break;
            case 'n':
                nseconds = atoi(optarg);     /* In fact, nseconds means number_of_hdus ?????????????????????????? */
                break;
            case 'P':
                prod_type = optarg[0];
                break;
    ...
    /* sanity checks */
    if (fscrunch_factor !=1 && fscrunch_factor !=2 && 
        fscrunch_factor !=4 && fscrunch_factor !=8 && fscrunch_factor !=16) {
    ...
    if (mode == 1 || mode == 0) {          /* 0: a raw correlator dump; 1: an NGAS FITS FILE */
        ...
        /* check that there are the same number of HDUs in each file and 
         * only extract the amount of time that there is actually data for */
        for (ifile=0; ifile < nfiles; ifile++) {
        ...
        /* if we're time averaging, check that an integer number of 
         * averages will go into output */
        if ((num_hdus_in_file[0]-primary)%tscrunch_factor != 0) {
            fprintf(stderr,"WARNING: There are %d time steps in file and " \
                    "time averaging of %d steps requested.\n",
        ...
        */
        nsta = (((int) round(sqrt(datadims[0]+1)))-1)/2;
        ninput = nsta*npol;
        nfreq = datadims[1]*nfiles;
        nvis = (nsta + 1)*(nsta/2)*npol*npol;
        if (debug) {
            fprintf(fpd,"Calculated there are %d stations in the data.",nsta);
            fprintf(fpd,"\tnpol: %d. nfreq: %d, nvis: %ld\n",npol,nfreq,(long)nvis);
        }

        matLength = nfreq * nvis; // cuda_matrix length (autos on the diagonal)
        lccspcLength = nfreq * (ninput-1)*ninput/2; //Lfile matrix length (no autos)
        lacspcLength = nfreq * ninput; //Lfile autos length
        fullLength = nfreq * ninput*ninput;

        printf("lccspcLength = %ld, lacspcLength = %ld\n", lccspcLength, lacspcLength);
        printf("matLength = %ld, fillLength = %ld\n", matLength, fullLength);
        ...
        if ((autos == NULL && (prod_type=='A'||prod_type=='B')) || 
            (cross == NULL && (prod_type=='B'||prod_type=='C'))) {
        ...
        //	 fill_mapping_matrix():
        //
        //   Output matrix full_matrix_h[] has ordering
        //	 corr_mapping[channel][station][station][polarization][polarization][complexity]

        fill_mapping_matrix(); // ======  FILL MAPPING MATRIX   FILL MAPPING MATRIX  for m == {0|1} =====


        /*
         * Dump the mapping matrix in binary format to file "mapmx.bin"
         */
        FILE *mapmx = fopen("mapmx.bin", "w");
        fwrite(corr_mapping, sizeof(map_t), NINPUT*NINPUT, mapmx);
        fclose(mapmx);
        ...
        while (!done_extracting) { // (ihdu <= stophdu)
            ...
            for (ifile = 0; ifile < nfiles; ifile++) {
                ...
                if (fits_movabs_hdu(fptr[ifile], ihdu, &hdutype, &status)) {
                    if (status == END_OF_FILE) {
                        done_extracting=1;  /* BREAK from (ifile = 0; ifile < nfiles; ifile++) loop ! */
                        break;
                    }
                    printerror(status);
                }
                ...
                if (hdutype == BINARY_TBL) {
                    ...
                    if (nfreq != (nrows*nfiles)) {
                        fprintf(stderr, "nfiles (%d) * nrows (%ld) not equal to nfreq (%d) " \
                                "are the FITS files the dimension you expected them to be?\n", \
                                nfiles,nrows,nfreq);
                        exit(EXIT_FAILURE);
                    }
                    ...
                    status = 0;
                    fits_read_col(fptr[ifile], typecode, 1, 1, 1, (repeat*nrows), &null,
                                  ptr, &anynull, &status);
                    if (status != 0 ) {
                    ...
                    if (nfreq != (naxes[1]*nfiles)) {
                        fprintf(stderr, "nfiles (%d) * nrows (%ld) not equal to nfreq (%d) " \
                                "are the FITS files the dimension you expected them to be?\n", \
                                nfiles,naxes[1],nfreq);
                        exit(EXIT_FAILURE);
                    }
                    ...
                    //printf(">>>>>>>>>>>>>>>>>> npixels=%ld,  matLength=%ld, fullLength=%ld\n\n", 
                    //                           npixels,  matLength, fullLength);

                    if (fits_read_img(fptr[ifile], TFLOAT, fpixel, npixels, &nullval,
                                      (float *)ptr, &anynull, &status)) {
                        printerror(status);
                    }
                } // else {  /*image extension */
            } // for (ifile = 0; ifile < nfiles; ifile++) 
            ...
            extractMatrix(full_matrix_h, cuda_matrix_h); //  ===  EXTRACT MATRIX   EXTRACT MATRIX  =
            ...
            for (input1 = 0; input1 < ninput; input1++) { // Real world input1
                for (input2 = input1 ; input2 < ninput; input2++) { // Real world input2 >= input1
            ...
            if ((n_averaged % tscrunch_factor) == 0) {
                ...
                /* 
                 * FWRITE FWRITE FWRITE FWRITE FWRITE FWRITE FWRITE FWRITE FWRITE FWRITE FWRITE
                 */

                if (autos !=NULL) fwrite(lac_accum, sizeof(float), \
                                         lacspcLength/fscrunch_factor, autos);
                if (cross !=NULL) fwrite(lcc_accum, sizeof(float complex),
                                         lccspcLength/fscrunch_factor, cross);
    ...
    else if (mode == 2) {            /* 2: Read an OLD format raw correlator dump */ 
        /* old format vis file */
        FILE *matrix;
        int rtn;
        float complex *cuda_matrix_h = NULL;
        ...
        // cuda_matrix length (autos on the diagonal)
        size_t matLength = nfreq * (ninput+npol)*ninput/2;
        // Lfile matrix length (no autos)
        size_t lccspcLength = nfreq * (ninput-1)*ninput/2; 
        size_t lacspcLength = nfreq * ninput; //Lfile autos length
        size_t fullLength = nfreq *ninput*ninput;
