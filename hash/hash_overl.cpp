...
public:
  ...
  Hash& operator[] (char *key);
  Hash& operator= (char *key);
  char *operator= (Hash&& hash);

private:
  size_t len;
  node_t **table;
  char *key; /* Temporary key address to be used with operator[] */
...
Hash& Hash::operator[] (char *key) { /* key is saved after applying ["key"] */
  this->key = key;
  return *this;
}

Hash& Hash::operator= (char *data) { /* Expressions like  h["key"] = "data"; */
  this->insert(this->key, data);
  return *this;
}

char *Hash::operator= (Hash&& hash) { /* Expressions like  str = h["key"]; */
...
int main() {
  ...
  Hash h1(100);
  h1.insert("alpha", "letter");
  h1.insert("pear", "fruit");
  //s = h1["alpha"];
  cout << s << endl;
  //h1["dog"] = "animal";
  //cout << "h[\"Campbell,Glenn\"] = " << h['Campbell,Glenn'] << endl;
  //cout << h['Campbell,Glenn'] << endl;
  //s = h['Campbell,Glenn'];
  //s = h1->["Campbell,Glenn"];
  //s = h1["pear"];
  //cout << s << endl;
