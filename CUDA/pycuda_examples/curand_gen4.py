...
__global__ void gen_proposal(float *pout, float *ptent, int *ptentn,
                             int nprm, int nitr, int nstates,
                             curandState *st) {
...
gen_proposal(cuda.In(pout), cuda.Out(ptent), cuda.Out(ptentn),
             nprm, nitr, np.int32(nstates), 
