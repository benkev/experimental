...
code = """
    ...
    __global__ void sim_drift(curandState *global_state, float const v,
                    float const V, float const a, float const z, float const Z,
                    float const t, float const T, float const dt,
                    float const intra_sv, float *out)
    {
        float start_delay, start_point, drift_rate, rand, prob_up, position,
              step_size, time;
...
def sim_drift(v, V, a, z, Z, t, T, size=512, dt=1e-4, update=False,
              return_gpu=False):
    ...
    _sim_drift_cuda(_generator.state, np.float32(v), np.float32(V), \
                    np.float32(a), np.float32(z), np.float32(Z), np.float32(t),\
                    np.float32(T), np.float32(dt), np.float32(1), _out, \
                    block=(64, 1, 1), grid=(size // 64 + 1, 1))
...
def sim_drift_var_thresh(v, V, a, z, Z, t, T, max_time, size=512, dt=1e-4, update=False, return_gpu=False):
    ...
    _sim_drift_var_thresh_cuda(_generator.state, np.float32(v), np.float32(V),\
                               _thresh_gpu, np.float32(z), np.float32(Z), \
                               np.float32(t), np.float32(T), np.float32(dt), \
                               np.float32(1), np.float32(max_time), _out, \
                               block=(64, 1, 1), grid=(size // 64 + 1,1))
...
def compute_stats_vec(data):
    ...
    for i, data_resp in enumerate([lower, upper]):
        ...
        # 7 quantiles
        stats_vec[i * 10 + 3:i * 10 + 9] = \
            stats_vec.extend(sp.stats.mstats.mquantiles(data_resp,
                             np.linspace(0,1,8, endpoint=False)[1:]))
...
def synth_likelihood(data, v, V, a, a_k, a_l, z, Z, t, T, samples=50,
                     drifts_per_sample=512):
    """Compute synthetic likelihood for collapsing threshold wfpt."""
    def mv_normal_like(s, mu, cov):
        s = np.asmatrix(s)
        mu = np.asmatrix(mu)
        cov = np.asmatrix(cov)
        return .5 * (s - mu) * (cov**-1) * (s - mu).T - .5 * \
    ...
    def _rvs(self, v, V, a, z, Z, t, T):
        ...
        if self.sampling_method == 'gpu':
            ...
            for i in np.round(np.arange(0, self._size, size)):
                sampled_rts = sim_drift(v, V, a, z, Z, t, T, size=size,
                                        dt=self.dt)
...
def main():
    ...
    #thresh_func = np.array(a*np.exp(-rate*np.linspace(0, max_time, steps)), \
    #                       dtype=np.float32)
    size = 412
    out = sim_drift_var_thresh(.5, .1, thresh_func, 0., .1, .3, .1, max_time, \
                               size=size, update=True)
...
if __name__ == '__main__':
    ...
    fill_normal(generator.state, out, block=(64, 1, 1), \
                grid=(size // 64 + 1, 1))
    data = out.get()
    print data.mean()
    print data.std()
    ...
    plt.plot(x, sampler.cdf(x, params['v'], params['V'], params['a'],
