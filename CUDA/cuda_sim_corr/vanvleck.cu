...
int load_rho_table(double *sig, double *rho, double (*rh)[NS][NR]) {
  ...
  fread(sig, NS*sizeof(double), 1, fh);
  fclose(fh);
  ...
  fread(rho, NR*sizeof(double), 1, fh);
  fclose(fh);
  ...
  fread(rh, NS*NS*NR*sizeof(double), 1, fh);
  fclose(fh);
...
int load_qsig_table(double (*qsig)[NQC]) {
  ...
  fread(qsig, NQS*NQC*sizeof(double), 1, fh);
  fclose(fh);
  return 0;
}  /* load_qsig_table() */
...
double inline lininterp(size_t n, double xx[], double yy[], double x) {
    /*
