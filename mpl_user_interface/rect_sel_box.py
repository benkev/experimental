...
def main():
	...
	def callback_for_Motion(event):                    # Callback for <Motion>
		global x, y
		kill_xhairs()
		
		dashes = [3, 2]
		x = canv.create_line(event.x, 0, event.x, 1000, dash=dashes, tags='xh')
		y = canv.create_line(0, event.y, 1000, event.y, dash=dashes, tags='xh')
        
    #
    # Called from cool_design(), the <Motion> callback
    #
	def kill_xhairs(event=None):
		canv.delete('xh')
	
	canv.bind('<Motion>', callback_for_Motion)    #, '+')
