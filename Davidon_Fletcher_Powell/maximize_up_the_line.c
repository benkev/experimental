////////////////////////////////////////////////////////////////////////////////
// File: maximize_up_the_line.c                                               //
// Routines:                                                                  //
//    Maximize_Up_the_Line                                                    //
////////////////////////////////////////////////////////////////////////////////
...
int Max_Search_Parabolic_Extrapolation(double (*f)(double), double x[],
               double fx[], double a, double b, double cut_off_scale_factor);
int Max_Search_Parabolic_Interpolation(double (*f)(double), double x[],
...
////////////////////////////////////////////////////////////////////////////////
//  int Maximize_Up_the_Line(double (*f)(double *), double x[], double fx,    //
...
//     this function maximizes g(p) = f(x + pv) for 0 < p <= cutoff. In       //
//     particular, if v is the gradient of f, then as long as v != 0, this    //
//     function searches for a maximum of f in the direction of the grad f.   //
//                                                                            //
//     This routine uses parabolic extrapolation to bracket a local maximum   //
...
//     double *p                                                              //
//        On output, *p is the value of p which maximizes f(x + pv) for p > 0.//
//     double v[]                                                             //
...
//        of p which maximizes f(x + pv).  The array y should be dimensioned  //
//        n in the calling routine.                                           //
//     double cutoff                                                          //
...
//                                                                            //
//     (your code to create vectors x, y, evaluate fx = f(x), set cutoff,     //
//      cutoff_scale_factor, and the tolerance )                              //
//                                                                            //
//     Minimize_Down_the_Line(f, x, fx, &p, v, y, cutoff,                     //
                          ...
                          double cutoff_scale_factor, double tolerance, int n)
{
   ...
   old_args = args;
   args = &g_args;
   g_lambda[0] = g(lambda[0]);
   g_lambda[2] = g(lambda[2]);
   args = &g_args;
   k = Max_Search_Parabolic_Extrapolation(g, lambda, g_lambda, 0.0, cutoff,
                                                       cutoff_scale_factor);
   if ( k < 0 ) k = 0;
   else { 
                        // Interpolate for the maximum 
     // The maximum is located between lambda[0] < lambda[1] < lambda[2],
     ...
     // i.e. f(x + lambda[0]*v) > f(x + lambda[1]*v) < f(x + lambda[2]*v).
      ...
      args = &g_args;
      k = Max_Search_Parabolic_Interpolation(g, lambda, g_lambda, tolerance );
