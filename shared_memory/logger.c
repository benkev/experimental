...
struct shared_memory { char buf [MAX_BUFFERS] [256]; 
    int buffer_index; 
...
int main (int argc, char **argv) { 
    ...
    // Open log file 
    if ((fd_log = open(LOGFILE, O_CREAT | O_WRONLY | O_APPEND | O_SYNC, 
                       0666)) == -1) 
        error ("fopen"); 
    ...
    if ((buffer_count_sem = sem_open (SEM_BUFFER_COUNT_NAME, O_CREAT, 
    ...
    while (1) { // forever 
        ...
        /* 
         * Since there is only one process (the logger) 
         * using the buffer_print_index, mutex semaphore is not necessary 
         */
        strcpy (mybuf, shared_mem_ptr -> 
                buf [shared_mem_ptr -> buffer_print_index]);  
