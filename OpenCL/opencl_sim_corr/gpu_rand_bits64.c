...
int main() {
    ...
    size_t source_size;
    fp = fopen("ker_rand_bits64.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to load kernel file.\n"); exit(1);
    }

    cl_ulong nrnd_total = Nwitem * Nwgroup * nrnd;
    cl_ulong *rndu = (cl_ulong *) malloc(nrnd_total*sizeof(cl_ulong));
    if (rndu == NULL) {
    ...
    cl_mem rndu_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, 
                          nrnd_total*sizeof(cl_ulong), NULL, &ret);
    ...
    /* Build the program */
    cl_char *opts = "-I .";
    // cl_char *opts = "-I /home/benkev/experimental/OpenCL/opencl_sim_corr";
